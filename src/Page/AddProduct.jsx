...
const AddProduct = ({ setProducts }) => {
  ...
  // Check if product data is passed for editing (e.g. state passed via navigate/Link)
  ...
  // Prefill price and other fields if editing an existing product
  useEffect(() => {
    if (existingProduct) {
      setFormData({
      ...
      if (existingProduct.image) {
        setPreviewUrls(prev => ({ ...prev, image: existingProduct.image }));
      }
    }
  }, [existingProduct]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let galleryUrls = existingProduct?.galleryUrls || [];

      // Only upload new images if selected
      if (files.image || Object.values(files).some(file => file !== null)) {
        const sigRes = await axios.get("https://super-store-backend-teal.vercel.app/api/v1/products/upload-signature");
        const signatureData = sigRes.data;

        if (files.image) {
          mainImageUrl = await uploadToCloudinary(files.image, signatureData);
        }
        ...
        if (newUploads.length > 0) {
          galleryUrls = [...galleryUrls, ...newUploads];
        }
      }
      ...
      if (isEditMode) {
        // Edit API Endpoint
        res = await axios.put(`https://super-store-backend-teal.vercel.app/api/v1/products/edit/${existingProduct._id}`, productData, {
          headers: { "Content-Type": "application/json" }
        });
      } else {
        // Add New Endpoint
        res = await axios.post("https://super-store-backend-teal.vercel.app/api/v1/products/add", productData, {
          headers: { "Content-Type": "application/json" }
        });
      }
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        if (setProducts) {
          setProducts(prev => 
            isEditMode 
              ? prev.map(p => p._id === existingProduct._id ? res.data.product : p)
      ...
      console.error("Upload/Edit error details:", error.response?.data || error);
      alert(error.response?.data?.message || error.response?.data?.error?.message || "Something went wrong during operation");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1 relative border-2 border-dashed border-gray-300 rounded-lg p-4 hover:bg-gray-50 transition-colors text-center cursor-pointer">
          <input 
            ...
            onChange={handleFileChange} 
            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
            required={!isEditMode && name === 'image'}
            disabled={loading}
          />
          <span className="text-gray-500 text-sm">
            {previewUrls[name] ? 'File Ready / Change' : 'Click to upload'}
          </span>
        </div>
        {previewUrls[name] && (
          <div className="w-16 h-16 shrink-0 rounded-lg border border-gray-200 overflow-hidden bg-gray-100 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 font-sans">
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-black py-6 px-8 flex justify-between items-center text-white">
          <h2 className="text-2xl font-bold">{isEditMode ? 'Edit Product Price & Details' : 'Add New Product'}</h2>
          <Link to="/admin" className="hover:text-gray-300 text-sm flex items-center gap-2">← Back</Link>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          ...
          <button 
            ...
            className={`w-full font-bold py-4 rounded-xl transition-all flex justify-center items-center gap-3 text-lg ${loading ? 'bg-gray-400' : 'bg-black text-white hover:bg-gray-800'}`}
          >
            {loading ? (isEditMode ? 'Updating...' : 'Uploading...') : (isEditMode ? 'Update Product' : 'Add Product')}
          </button>
        </form>
      </div>
    </div>
