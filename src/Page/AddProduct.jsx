...
const AddProduct = ({ setProducts }) => {
  ...
  //
  // IMPORTANT:
  // We DO NOT compress or resize the image here.
  // The original file goes directly to Cloudinary.
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // Convert Cloudinary URL to optimized delivery URL
  //
  // f_auto = automatic format (WebP/AVIF when supported)
  // q_auto = automatic quality optimization
  //
  // This does NOT modify the original uploaded file.
  // ----------------------------------------------------
  ...
  const uploadToCloudinary = async (file, signatureData) => {
    ...
    // Automatically determine Cloudinary resource type
    ...
    if (!response.data?.secure_url) {
      throw new Error(
        'Cloudinary upload failed: secure URL not received.'
      );
    }

    // Return optimized delivery URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const signatureResponse = await axios.get(
        'https://super-store-backend-teal.vercel.app/api/v1/products/upload-signature'
      );
      ...
      const galleryFiles = galleryKeys.filter(
        (key) => files[key]
      );


      const uploadPromises = galleryFiles.map((key) =>
        uploadToCloudinary(
          files[key],
          signatureData
        )
      ...
      // ------------------------------------------------
      const response = await axios.post(
        'https://super-store-backend-teal.vercel.app/api/v1/products/add',
        productData,
        {
          headers: {
      ...
      if (response.status === 201) {
        ...
        if (setProducts && response.data?.product) {
          setProducts((prev) => [
